...
void BleKeyboard::begin(void)
{
  ...
  // [SecureKey] bond=true, mitm=false, sc=true → LE Secure Connections
  // "Just Works". mitm=true (the library default) demanded a passkey this
  // display-less device can't show, so Windows/Android paired but never typed.
  // sc=false (legacy) fixed those but macOS/iOS refuse to enable HID over a
  // legacy link — so we keep sc=true here, which all modern hosts accept.
